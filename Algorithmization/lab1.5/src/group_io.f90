...
contains
   ...
   recursive function Read_student(In) result(Stud)
      ...
      allocate (Stud)
      format = '(4(a, 1x), f5.2)'
      read (In, format, iostat=IO) stud%Surname, stud%Initials, stud%Sex, stud%Registration, stud%Aver_Mark
   ...
   recursive subroutine Output_student(Out, Stud)
      ...
      format = '(4(a, 1x), f5.2)'
      write (Out, format, iostat=IO) Stud%Surname, Stud%Initials, Stud%Sex, Stud%Registration, Stud%Aver_Mark
